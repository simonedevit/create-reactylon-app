...
(async () => {
    ...
    if (isNative) {
        ...
        exec(`npx @react-native-community/cli@latest init ${projectName} --version 0.74.2 --skip-install --skip-git-init`, async (error) => {
            ...
            spinner.start();
            exec('npm i @babylonjs/core@7.40.2 @babylonjs/loaders@7.40.2 @babylonjs/react-native @babylonjs/react-native-iosandroid-0-71 react-native-permissions@^3.10.1 reactylon react-reconciler@0.29.2 --save --force', { cwd: projectPath }, (error) => {
                if (error) {
                    spinner.fail();
                    console.error(error);
                    return;
